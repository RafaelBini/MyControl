    ...
    class TransacaoDAO
    {
        public static void Transferir(string contaDoadora, string contaRecebedora, string valor)
        {
            ...
            string q2 = "insert into transacao (descricao,valor,codconta,datapgto,tipo,adddatetime,metodoentrada,conta) " +
                        "values ('Transferência entre contas MyControl (cod " + agora.ToString("yyyyMMddHHmmss") + ")', '" + valor.Replace(",", ".") + "', (select codconta from conta where nome='" + contaRecebedora + "'), '" + DateTime.Today.ToString() + "', 'D', '" + agora.ToString() + "', 'C# MyC', '" + contaRecebedora + "'); ";
